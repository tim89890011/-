 ...
 */
const _isMobileCoin = /Mobi|Android|iPhone|iPad/i.test(navigator.userAgent);
...
function showSignalNotification(signal) {
    ...
    const cssClass = ['BUY','COVER'].includes(sig) ? 'buy' : ['SELL','SHORT'].includes(sig) ? 'sell' : 'hold';
    const sigCn = { BUY: '买入', SELL: '卖出', HOLD: '观望', SHORT: '做空', COVER: '平空' }[sig] || '观望';
    const reason = signal.final_reason || 'AI生成交易信号';

    const notif = document.createElement('div');
    notif.className = `signal-notification ${cssClass}`;
    notif.innerHTML = `
        <div class="signal-icon"><i class="ri-bar-chart-2-line"></i></div>
        <div class="signal-content">
            <div class="signal-title">${escapeHtml(signal.symbol || '未知')} - ${sigCn}信号</div>
            <div class="signal-desc">${escapeHtml(reason)}</div>
        </div>
    `;
