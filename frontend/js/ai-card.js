...
export function renderSignalHero(container, signal) {
    const sig = signal.signal || 'HOLD';
    const sigLower = sig.toLowerCase();
    const sigCn = { BUY: '买入', SELL: '卖出', HOLD: '观望', SHORT: '做空', COVER: '平空' }[sig] || '观望';
    ...
    container.innerHTML = `
    <div class="ai-hero signal-${sigLower} hero-bounce">
        <div class="ai-hero-top">
            <div class="ai-hero-signal">
...
export async function loadRecentSignals() {
    ...
    try {
        ...
        // 显示 AI 最后分析时间
        const lastAt = data.last_analysis_at;
        let aiStatusHtml = '';
        if (lastAt) {
            const ago = Math.round((Date.now() - new Date(lastAt).getTime()) / 60000);
            const agoText = ago < 1 ? '刚刚' : ago < 60 ? `${ago}分钟前` : `${Math.round(ago / 60)}小时前`;
            aiStatusHtml = `<div style="font-size:11px;color:var(--text3);padding:6px 14px;display:flex;align-items:center;gap:6px;border-bottom:1px solid var(--border);">
                <span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:var(--green);animation:pulse 2s infinite;"></span>
                AI 持续分析中 · 最后分析 ${agoText}${ago < 10 ? '' : ' · 当前市场均为观望信号'}
            </div>`;
        }

        if (!data.history || data.history.length === 0) {
            container.innerHTML = aiStatusHtml + '<div class="no-data">暂无买入/卖出信号，AI 持续分析中...</div>';
            return;
        }

        container.innerHTML = aiStatusHtml + data.history.map(s => {
            const dotCls = s.signal === 'BUY' ? 'dot-buy' : s.signal === 'SELL' ? 'dot-sell' : 'dot-hold';
            const sigCn = { BUY: '买入', SELL: '卖出', HOLD: '观望', SHORT: '做空', COVER: '平空' }[s.signal] || '观望';
